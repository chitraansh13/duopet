...
export function FoodHistoryPanel({ duoId, userId, today, timezone, history, diaryVisible, totalsVisible, partner }: {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      setLoading(false);
    }).catch(() => { if (live) { setEntries([]); setTotals(null); setLoading(false); setError("This food day couldn’t be loaded. Please retry."); } });
    return () => { live = false; };
  ...
  return <div className="space-y-4">
    ...
    {totalsVisible && <section className="grid grid-cols-2 gap-3" aria-label={`${date} nutrition totals`}>
      <div className="rounded-[1.25rem] bg-surface p-4 shadow-soft"><p className="text-xs font-bold text-muted">Calories</p><p className="mt-2 text-xl font-bold">{loading ? "…" : formatNutrition(totals?.calories ?? 0,"kcal")}</p></div>
      <div className="rounded-[1.25rem] bg-surface p-4 shadow-soft"><p className="text-xs font-bold text-muted">Protein</p><p className="mt-2 text-xl font-bold">{loading ? "…" : formatNutrition(totals?.protein ?? 0,"g")}</p></div>
    </section>}
