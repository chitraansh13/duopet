...
export function ManageGoals({ initialId }: { initialId?: string }) {
  const { goals, addGoal, updateGoal, deleteGoal, setStatus } = useGoals();
  ...
  function save(goal: GoalDefinition) {
    if (editing) updateGoal(goal); else addGoal(goal);
  ...
  return (
    <div className="space-y-5 py-4 sm:space-y-6 sm:py-7">
      ...
      <div className="grid items-start gap-5 lg:grid-cols-[minmax(320px,.75fr)_minmax(0,1.25fr)]">
        <section className="rounded-[1.5rem] bg-surface p-3 shadow-soft"><h2 className="px-2 py-2 text-sm font-bold">Your manageable goals</h2><div className="divide-y divide-line">{manageable.map((goal) => <div key={goal.id} className="flex items-center gap-3 px-2 py-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-subtle text-muted"><GoalIcon name={goal.icon} /></span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{goal.name}</p><p className="mt-0.5 text-[10px] capitalize text-muted">{goal.scope} · {goal.status}</p></div><button type="button" onClick={() => setStatus(goal.id, goal.status === "active" ? "paused" : "active")} aria-label={`${goal.status === "active" ? "Pause" : "Resume"} ${goal.name}`} className="grid size-10 place-items-center rounded-full text-luxury hover:bg-luxury-soft">{goal.status === "active" ? <Pause className="size-4" /> : <Play className="size-4" />}</button><button type="button" onClick={() => { setEditingId(goal.id); setCreating(false); }} aria-label={`Edit ${goal.name}`} className="grid size-10 place-items-center rounded-full text-muted hover:bg-subtle"><Edit3 className="size-4" /></button><button type="button" onClick={() => setDeleting(goal)} aria-label={`Delete ${goal.name}`} className="grid size-10 place-items-center rounded-full text-accent hover:bg-accent-soft"><Trash2 className="size-4" /></button></div>)}</div></section>
      ...
      <AnimatePresence>{deleting && <DeleteGoalDialog goal={deleting} onCancel={() => setDeleting(null)} onDelete={() => { deleteGoal(deleting.id); if (editingId === deleting.id) { setEditingId(null); setCreating(true); } setDeleting(null); }} />}</AnimatePresence>
    </div>
  );
}
