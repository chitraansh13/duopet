...
import { formatGoalValue, getGoalTarget, type GoalDefinition, type GoalTarget } from "@/lib/goal-data";
import { GoalIcon } from "@/components/goals/GoalIcon";
import { GoalProgress } from "@/components/goals/GoalProgress";

function historyLabel(goal: GoalDefinition, target: GoalTarget, offset: number) {
  if (goal.trackingType === "boolean") return offset === 2 ? "Pending" : "Completed ✓";
  const value = offset === 0 ? target.currentValue : offset === 1 ? target.target ?? 0 : (target.target ?? 0) * 0.88;
  return `${formatGoalValue(value, goal.unit)} / ${formatGoalValue(target.target ?? 0, goal.unit)}${value >= (target.target ?? 0) ? " ✓" : ""}`;
}
...
export function TaskDetail({ goal, onClose }: { goal: GoalDefinition; onClose: () => void }) {
  ...
  const historyTarget = you ?? friend!;
  return (
    <aside className="fixed inset-x-3 bottom-[calc(6.5rem+env(safe-area-inset-bottom))] top-[max(1rem,env(safe-area-inset-top))] z-40 overflow-y-auto rounded-[1.6rem] bg-elevated p-5 shadow-card lg:static lg:max-h-[calc(100vh-8rem)] lg:rounded-[1.5rem]" aria-label={`${goal.name} details`}>
      ...
      <div className="mt-6"><div className="flex items-center gap-2"><CalendarClock className="size-4 text-luxury" /><h3 className="text-sm font-bold">Recent history</h3></div><div className="mt-2 divide-y divide-line rounded-2xl bg-surface px-4 shadow-soft">{["Today", "Yesterday", "Saturday"].map((day, index) => <div key={day} className="flex items-center justify-between gap-3 py-3 text-xs"><span className="text-muted">{day}</span><span className="font-semibold">{historyLabel(goal, historyTarget, index)}</span></div>)}</div></div>
