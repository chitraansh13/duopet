...
import { taskHistory } from "@/lib/goal-history";
...
export function TaskDetail({ goal, onClose }: { goal: GoalDefinition; onClose: () => void }) {
  ...
  return (
    <aside ref={panel} tabIndex={-1} onKeyDown={(event) => { if (event.key === "Escape") onClose(); }} className="glass-panel fixed inset-x-3 bottom-[calc(6.5rem+env(safe-area-inset-bottom))] top-[max(1rem,env(safe-area-inset-top))] z-40 overflow-y-auto rounded-[1.6rem] p-5 lg:static lg:max-h-[calc(100vh-8rem)] lg:rounded-[1.5rem]" aria-label={`${goal.name} details`}>
      ...
      <div className="mt-6"><div className="flex items-center gap-2"><CalendarClock className="size-4 text-luxury" /><h3 className="text-sm font-bold">Recent history</h3><span className="rounded-full bg-subtle px-2 py-1 text-[9px] font-semibold text-muted">Demo preview</span></div><p className="mt-1 text-[11px] text-muted">Today is live. Earlier history is not connected in this phase.</p><div className="mt-2 divide-y divide-line rounded-2xl bg-surface px-4 shadow-soft">{taskHistory(goal, historyTarget).map((day) => <div key={day.label} className="flex items-center justify-between gap-3 py-3 text-xs"><span className="text-muted">{day.label}</span><span className="font-semibold">{day.value}</span></div>)}</div></div>
