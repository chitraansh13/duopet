...
export function GoalForm({ initial, onSubmit, onCancel }: { initial?: GoalDefinition; onSubmit: (goal: GoalDefinition) => void; onCancel?: () => void }) {
  ...
  const [yourTarget, setYourTarget] = useState(initial?.targets.find((target) => target.userId === "you")?.target ?? 1);
  const [friendTarget, setFriendTarget] = useState(initial?.targets.find((target) => target.userId === "friend")?.target ?? 1);
  ...
  function submit(event: React.FormEvent) {
    ...
    const users: UserId[] = scope === "shared" ? ["you", "friend"] : ["you"];
    const sameTracking = initial?.trackingType === trackingType;
    onSubmit({
      ...
      targets: users.map((userId) => ({
        userId,
        target: measured ? (userId === "you" ? yourTarget : friendTarget) : undefined,
  ...
  return (
    <form onSubmit={submit} className="rounded-[1.6rem] bg-surface p-5 shadow-card sm:p-6">
      ...
      <div className="mt-6 flex gap-3">{onCancel && <button type="button" onClick={onCancel} className="min-h-12 flex-1 rounded-xl bg-subtle px-4 text-sm font-bold text-muted">Cancel</button>}<button type="submit" className="min-h-12 flex-1 rounded-xl bg-accent px-4 text-sm font-bold text-on-accent">{initial ? "Save changes" : "Add goal"}</button></div>
    </form>
  );
}
