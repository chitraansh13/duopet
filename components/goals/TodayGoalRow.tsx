  ...
  return (
    <article className="rounded-[1.25rem] bg-surface p-3.5 shadow-soft">
      <div className="flex items-center gap-3">
        ...
        {goal.trackingType === "boolean" ? <button type="button" onClick={() => setProgress(goal.id, "you", complete ? 0 : 1)} aria-label={`${complete ? "Uncheck" : "Complete"} ${goal.name}`} aria-pressed={complete} className="grid size-11 place-items-center rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/40">{complete ? <Check className="animate-check-pop size-6 rounded-full bg-accent p-1 text-on-accent" /> : <Circle className="size-6 text-muted" />}</button> : <button type="button" onClick={() => setEditing((value) => !value)} aria-expanded={editing} aria-label={`Update ${goal.name}`} className="grid size-11 place-items-center rounded-full text-muted hover:bg-subtle"><ChevronDown className={`size-4 transition-transform ${editing ? "rotate-180" : ""}`} /></button>}
      </div>
      ...
      <AnimatePresence initial={false}>{editing && goal.trackingType === "measured" && <MeasuredProgressEditor goal={goal} target={you} onChange={(value) => setProgress(goal.id, "you", value)} onClose={() => setEditing(false)} />}</AnimatePresence>
    </article>
  );
}
