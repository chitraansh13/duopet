...
export function ChallengesDashboard() {
  ...
  return <MotionConfig reducedMotion="user" transition={{ duration: 0.35, ease: "easeOut" }}><div className="space-y-5 py-4 sm:space-y-6 sm:py-7">
    ...
    <section><div className="mb-4 flex items-end justify-between"><div><p className="text-xs font-semibold text-accent">In motion</p><h2 className="mt-1 text-xl font-bold tracking-tight sm:text-2xl">Active challenges</h2></div>{active.length>0&&<p className="text-xs text-muted">Tap for details</p>}</div><motion.div layout className="grid items-start gap-4 lg:grid-cols-2"><AnimatePresence initial={false}>{active.map((challenge) => { const goal = goals.find((item) => item.id === challenge.linkedGoalId)!; return <ChallengeCard key={challenge.id} challenge={challenge} goal={goal} onOpen={() => setSelected(challenge)} />; })}</AnimatePresence>{active.length===0&&<div className="rounded-[24px] bg-surface p-8 text-center shadow-soft lg:col-span-2"><p className="font-bold">No challenges yet.</p><p className="mt-2 text-sm text-muted">Your first shared challenge will appear here.</p></div>}</motion.div></section>
  ...
  {runtime.isDemoMode&&<ChallengeSheet open={creatorOpen} onClose={() => setCreatorOpen(false)} onCreate={createChallenge} />}
