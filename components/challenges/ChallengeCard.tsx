...
export function ChallengeCard({ challenge, goal, onOpen }: { challenge: Challenge; goal: GoalDefinition; onOpen: () => void }) {
  ...
  const progress = deriveChallengeProgress(challenge, goal);
    ...
    <motion.article layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="overflow-hidden rounded-[24px] bg-surface shadow-card">
      <button type="button" onClick={onOpen} className="w-full p-5 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-accent/40 sm:p-6">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1"><span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold ${together ? "bg-accent-soft text-accent" : "bg-[var(--friend-soft)] text-friend"}`}>{together ? <UsersRound className="size-3" /> : <Swords className="size-3" />}{together ? "Together" : "Head to Head"}</span><h3 className="mt-2 text-lg font-bold tracking-tight">{challenge.name}</h3><p className="mt-1 text-xs text-muted">{goal.name} · {challengeRemaining(challenge)}</p></div>
        </div>
        ...
        <div className="mt-4 flex items-center justify-between gap-3 border-t border-line pt-4 text-xs text-muted"><span className="inline-flex items-center gap-1.5"><Timer className="size-3.5" />{challengeRemaining(challenge)}</span><span className="inline-flex min-w-0 items-center gap-1.5"><Gift className="size-3.5 shrink-0 text-luxury" /><span className="truncate">{challenge.reward || "Just for fun"}</span></span></div>
      </button>
    </motion.article>
  );
}
