...
export function AccessoryPicker({ items, selected, onSelect, disabled=false }: { items: Accessory[]; selected: DogAccessory; onSelect: (id: DogAccessory) => void; disabled?:boolean }) {
    ...
    <section className="rounded-[24px] bg-surface p-5 shadow-card sm:p-6">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3">
        {items.map((item) => { const Icon = accessoryIcons[item.type]; const active = selected === item.id; return (
          <motion.button key={item.id} type="button" disabled={disabled} onClick={() => choose(item)} animate={lockedNotice === item.id ? { x: [0, -3, 3, -2, 0] } : {}} aria-pressed={item.unlocked ? active : undefined} className={`min-h-24 rounded-[18px] p-3 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/40 disabled:opacity-60 ${active ? "bg-accent-soft" : "bg-subtle hover:bg-elevated"}`}>
...
export function RoomItemPicker({ items, activeItems, onToggle, disabled=false }: { items: RoomItem[]; activeItems: string[]; onToggle: (id: string) => void; disabled?:boolean }) {
