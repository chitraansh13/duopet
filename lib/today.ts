...
export function deriveToday(goals: GoalDefinition[], currentUserId: UserId, partnerUserId: UserId, companion: CompanionSnapshot, partnerNutritionVisible = true) {
  ...
  const partnerGoals = activeGoals.filter((goal) => goal.scope === "personal" && getGoalTarget(goal, partnerUserId));
  const youProgress = userDailyProgress(activeGoals, currentUserId);
  const friendProgress = userDailyProgress(activeGoals.filter((goal) => partnerNutritionVisible || goal.progressSource === "manual"), partnerUserId);
