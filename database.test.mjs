...
const sharedTargets=JSON.stringify(ids.slice(0,2).map(user_id=>({user_id,target:10})));
...
await reject("insert into goal_assignments(goal_id,user_id,target_value,canonical_unit,active_from) values($1,$2,20,'pages',current_date)",[goal,ids[0]]);
...
await reject('insert into goal_checkins(goal_id,user_id,local_date,value) values($1,$2,current_date,1)',[personal,ids[1]]);
await as(0);
await db.query('select set_goal_checkin($1,current_date,10)',[goal]);
await reject('insert into goal_checkins(goal_id,user_id,local_date,value) values($1,$2,current_date,10)',[goal,ids[1]]);
...
assert.equal(Number((await one('select target_value from goal_assignments where goal_id=$1 and user_id=$2 and active_until is null',[goal,ids[0]])).target_value),20); checks++;
...
await db.query('select set_goal_checkin($1,current_date,5)',[goal]);
const challenge=(await one("insert into challenges(duo_id,linked_goal_id,name,mode,metric,target,start_date,end_date,created_by) values($1,$2,'Read together','together','target_days',5,current_date,current_date+6,$3) returning id",[duo,goal,ids[1]])).id;
await reject("insert into pet_xp_events(duo_id,source_type,source_id,event_key,xp_amount,local_date) values($1,'perfect_day',$1,'test',10,current_date)",[duo]);
...
await reject('select set_goal_checkin($1,current_date,1)',[goal]);
await db.exec('reset role');
await db.query("insert into pet_xp_events(duo_id,source_type,source_id,event_key,xp_amount,local_date) values($1,'perfect_day',$1,$2,10,current_date)",[duo,`perfect_day:${duo}:${new Date().toLocaleDateString('en-CA',{timeZone:'UTC'})}`]);
...
assert.equal((await one('select sum(xp_amount)::int n from pet_xp_events where reversed_at is null')).n,10); checks++;
